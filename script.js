...
const VISIBILITY_DISTANCE = 30;   // meters — chest visible within this distance
const CAPTURE_DISTANCE    = 10;    // meters — can capture/open within this distance

// FIXED BASE COORDINATES (reference point)
const BASE_LATITUDE = 28.412090;
const BASE_LONGITUDE = 77.109285;
...
AFRAME.registerComponent('chest-animation', {
  ...
  playChestAnimation: function () {
    ...
    if (this.model) {
      ...
      const direction = new THREE.Vector3().subVectors(modelWorldPos, worldCameraPos);
      ...
      console.log('Camera angle:', (angle * 180 / Math.PI).toFixed(2), 'degrees');
      console.log('Chest rotation:', (this.model.rotation.y * 180 / Math.PI).toFixed(2), 'degrees');
    }
...
window.onload = () => {
  ...
  // Calculate fixed treasure location - 5m NORTH from base coordinates
  treasureLocation = offsetLatLng(BASE_LATITUDE, BASE_LONGITUDE, 0, 0); // 0 degrees = North
  
  console.log('📍 Base coordinates:', BASE_LATITUDE, BASE_LONGITUDE);
...
function waitForGPSAndStart() {
  ...
  console.log('✅ Initial GPS acquired from camera:', gps);
  ...
  console.log('📍 Current GPS set:', currentUserGPS);

  showTreasure();
  
  // Start high-accuracy GPS tracking after initial load
  startHighAccuracyGPS();
}

function startHighAccuracyGPS() {
  if (!navigator.geolocation) {
    console.warn('⚠️ Geolocation not supported, falling back to camera GPS');
    return;
  }
  ...
  const watchId = navigator.geolocation.watchPosition(
    (position) => {
      // Update current GPS position with high accuracy
      currentUserGPS = {
        latitude: position.coords.latitude,
        longitude: position.coords.longitude
      };
      
      console.log('📍 GPS updated (high-accuracy):', 
        currentUserGPS.latitude.toFixed(6), 
        currentUserGPS.longitude.toFixed(6),
  ...
  console.log('✅ High-accuracy GPS tracking started, watch ID:', watchId);
}

function startGPSTracking() {
  setInterval(() => {
    if (!currentUserGPS) return;

    // Update GPS display
    const gpsInfo = document.getElementById('gps-info');
    if (gpsInfo) {
      const lat = currentUserGPS.latitude.toFixed(6);
      const lng = currentUserGPS.longitude.toFixed(6);
      gpsInfo.innerHTML = `${lat}<br>${lng}`;
    }
...
function showTreasure() {
  ...
  if (isCaptured) {
    console.log('⚠️ Treasure already captured!');
    return;
  }

  console.log('💎 Creating treasure entity...');

  const entity = document.createElement('a-entity');

  entity.setAttribute('gps-entity-place', `latitude: ${treasureLocation.lat}; longitude: ${treasureLocation.lng}`);
  ...
  entity.addEventListener('model-error', (e) => {
    console.error('❌ Error loading treasure model:', e);
  });
  ...
  console.log(`✅ Treasure placed at: ${treasureLocation.lat.toFixed(6)}, ${treasureLocation.lng.toFixed(6)}`);
}
...
function tryCapture(event) {
  ...
  console.log(`Distance to treasure: ${distance.toFixed(2)}m (need to be within ${CAPTURE_DISTANCE}m)`);
...
function offsetLatLng(lat, lng, meters, bearingDeg) {
  ...
  const newLng = lng + (meters * Math.sin(brng)) / (R * Math.cos(latRad)) * (180 / Math.PI);
